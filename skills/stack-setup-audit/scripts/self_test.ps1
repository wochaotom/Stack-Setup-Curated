...
Add-Check "external skill discovery gated" ($skill -match "Codex/OpenAI skill sources" -and $skill -match '\$skill-installer' -and $skill -match "discovery-only index")
Add-Check "unofficial mirrors not qualified" ($skill -match "unaffiliated directories or mirrors" -and $skill -match "vetted source authority")
Add-Check "standard and upstream references" ($skill -match "Agent Skills standard" -and $skill -match "Anthropic / Claude Code official docs" -and $skill -match "GitHub Copilot agent skills docs")
...
Add-Check "community source review gated" ($skill -match "community indexes" -and $skill -match "Pin external skills")
...
Add-Check "source review scorecard present" ($skill -match "Source Review Scorecard" -and $skill -match "runtime surface" -and $skill -match "permission class" -and $skill -match "maintenance/update cadence" -and $skill -match "data sensitivity" -and $skill -match "local outcome evidence" -and $skill -match "fit decision" -and $skill -match "verification path")
Add-Check "harness evaluation loop present" ($skill -match "Harness Evaluation Loop" -and $skill -match "source-review decisions" -and $skill -match "verifier output" -and $skill -match "local outcome evidence" -and $skill -match "smallest failing surface")
...
Add-Check "audit emits safe source policy" ($auditText -match "Safe Source Policy" -and $auditText -match "unaffiliated directories or mirrors")
...
Add-Check "audit emits source review scorecard" ($auditText -match "Source Review Scorecard" -and $auditText -match "sourceAuthority" -and $auditText -match "permissionClass" -and $auditText -match "maintenanceCadence" -and $auditText -match "dataSensitivity" -and $auditText -match "localOutcomeEvidence" -and $auditText -match "conversionLoss")
Add-Check "audit emits install bloat signals" ($auditText -match "Install Bloat Signals" -and $auditText -match "Broad marketplace bundle" -and $auditText -match "No owner")
...
Add-Check "audit emits harness evaluation loop" ($auditText -match "Harness Evaluation Loop" -and $auditText -match "Trace:" -and $auditText -match "Outcome:" -and $auditText -match "Iteration:")
...
Add-Check "converter emits source review" ($conversionJson.sourceReview.requiredBeforeInstall -eq $true -and $conversionJson.sourceReview.permissionClass -match "local write" -and $conversionJson.sourceReview.maintenanceCadence -match "unknown" -and $conversionJson.sourceReview.dataSensitivity -match "unknown" -and @($conversionJson.sourceReview.localOutcomeEvidence).Count -gt 0 -and @($conversionJson.sourceReview.runtimeSurface | Where-Object { $_ -eq "sourceSkill.supportingFileCount=0" }).Count -eq 1 -and @($conversionJson.sourceReview.verificationPath).Count -gt 0)
