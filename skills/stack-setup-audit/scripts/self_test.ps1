...
Add-Check "source review scorecard present" ($skill -match "Source Review Scorecard" -and $skill -match "runtime surface" -and $skill -match "permission class" -and $skill -match "fit decision" -and $skill -match "verification path")
Add-Check "harness evaluation loop present" ($skill -match "Harness Evaluation Loop" -and $skill -match "source-review decisions" -and $skill -match "verifier output" -and $skill -match "smallest failing surface")
...
Add-Check "audit emits source review scorecard" ($auditText -match "Source Review Scorecard" -and $auditText -match "sourceAuthority" -and $auditText -match "permissionClass" -and $auditText -match "conversionLoss")
...
Add-Check "audit emits harness evaluation loop" ($auditText -match "Harness Evaluation Loop" -and $auditText -match "Trace:" -and $auditText -match "Iteration:")
...
Add-Check "converter emits source review" ($conversionJson.sourceReview.requiredBeforeInstall -eq $true -and $conversionJson.sourceReview.permissionClass -match "local write" -and @($conversionJson.sourceReview.verificationPath).Count -gt 0)
